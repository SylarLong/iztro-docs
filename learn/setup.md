...
:::warning 注意
流曜一般只取 `大限` 和 `流年` 的星曜，流月、流日、流时的一般不看，所以本站程序也只提供到大限和流年的流曜。一般来说，`大限`的流曜会在星曜前面加上 `运` 字，`流年` 的流曜会在星曜前面加上 `流` 字，以便区分。比如大限的 `天马` 叫 `运马`，`流年` 的 `天马` 叫 `流马`。
:::
