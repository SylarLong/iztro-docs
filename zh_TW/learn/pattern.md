...
這個格局的形式比較多，只要命宮三方四正湊齊天機、太陰、天同、天梁這四顆星即可。由於這個格局的條件比較寬松，所以非常容易湊齊，所以形成這個格局的概率是非常之高的。它有以下八種情況可以滿足這個格局，其中有部分是需要借對宮的星曜來形成：
...
除了上面的格局，理論上講任何太陽星和太陰星廟旺出現在命宮三方四正的，都應該算日月並明格。但是不同的組合，其實個性和為人處事上是差別很大的，所以並不能夠以一個格局名字定高低。而且，按照傳統的成格條件，當然是煞星越少越好，吉星越多越好，相信絕大多數的星曜組合，在這種情況下都不會太差。比如如下布局，也是日月並明格，不過它有另外一個名字，叫做 [丹墀桂墀格](#丹墀桂墀)：
...
破軍星確實有沖破天羅地網的那種魄力，然而在克服重重困難以後，是否一定能取得好的結果，這是說不準的。當然，破軍星這種敢於破斧成舟，放手一博的個性，加上環境和內心的磨礪，確實是更加容易比其他星曜更加容易有大收獲。但是這是存在幸存者偏差的，大部分人的努力，可能不一定能換回來一個好的結果。

以下是日月照壁的星曜分布，在鏡像位置上還有一個，此處就不列出來了：
...
筆者多次建議大家不要以格局來定高低，任何星曜，任何組合，任何格局都是有兩面性的，我們應該站在中立的角度去分析它們的特點。就比如同樣是交通工具，車、船、飛機、火車各有各的特點，我們不能說誰比誰要優越。
...
太陽和太陰只有當命宮在 `丑宮` 和 `未宮` 的時候才會產生夾著命宮的情況，而此時只有兩種星曜組合能夠滿足，也就是命宮主星是 [天府星](major-star.md#天府星) 或者 [武曲貪狼](major-star.md#武曲貪狼) 的時候能夠產生日月夾命的格局。當然，需要成格是需要命宮沒有 [空亡星](adj-star.md#空亡星) 並且有吉星同度。
...
其效應和日月夾命大同小異，只是更偏向於財。就星曜分布而言，也和日月夾命是一樣的，只是把命宮換成財帛宮即可，此處不再贅述。
...
從星曜特性上來說，本來文昌武曲應該是能文能武，兼具文人墨客的氣節和武者的豪氣。但是多了鈴星和陀羅的牽制，使這個組合變成了文不成武不就的感覺。換一個角度來理解，也是那種懷才不遇的感覺。細想一下，武曲的剛烈，文昌的清高，陀羅的鑽牛角尖和鈴星的反覆混在一起，是個什麼感覺？
...
這個格局能找到的資料不多，古書上面也很少提及這個格局。從星曜組合上來分析，也不難知道為什麼它叫這個名字。空亡之星也可以理解為「空忙」之星，加之廉貞星反反覆復的特點，就會給人一種老是在一頓猛如虎的操作後，得到的回報卻達不到自己的預期目標，從而產生挫敗感。


那為什麼是廉貞星呢？這是因為別的星曜更容易在多次嘗試未果後選擇放棄，而廉貞更加的容易產生「我不甘心」的心態。如果有讀者是這個格局，歡迎 [添加筆者微信](/about.md) 來講出你的故事。
...
不難看出，這是個備受歡迎的格局，畢竟誰不想食祿千鍬呢？但是得到這個格局一定會食祿千鉺麼？我看也未必。不過府相都是穩定性非常高的星曜，所以也造就了命主比較容易形成一個四平八穩的個性。那為什麼說命宮在寅申的時候要優越一點呢？這是因為命宮在寅申的時候，本格是 [雄宿朝元格](#雄宿朝元)。
...
<table class="astrolabe">
 <tr>
 <td><div class="pattern"><div></div><div><code>巳</code> <code>僕人</code></div></div></td>
 <td><div class="pattern"><div>天機</div><div><code>午餐</code> <code>遷移</code></div></div></td>
 <td><div class="pattern"><div>紫微 破軍</div><div><code>未</code> <code>疾厄</code></div></div></td>
 <td><div class="pattern"><div></div><div><code>申</code> <code>財帛</code></div></div></td>
 </tr>
 <tr>
 <td><div class="pattern"><div>太陽</div><div><code>辰</code> <code>官祿</code></div></div></td>
 <td class="center-palace" rowspan="2" colspan="2">石中隱玉</td>
 <td><div class="pattern"><div>天府</div><div><code>酉</code> <code>子女</code></div></div></td>
 </tr>
 <tr>
 <td><div class="pattern"><div>武曲 七殺</div><div><code>卯</code> <code>田宅</code></div></div></td>
 <td><div class="pattern"><div>太陰</div><div><code>戌</code> <code>夫妻</code></div></div></td>
 </tr>
 <tr>
 ...
 <td><div class="pattern"><div>巨門</div><div><code>子</code> <code>命宮</code></div></div></td>
 <td><div class="pattern"><div>廉貞 貪狼</div><div><code>亥</code> <code>兄弟</code></div></div></td>
 </tr>
</table>
...
<table class="astrolabe">
 <tr>
 <td><div class="pattern"><div>太陽</div><div><code>巳</code> <code>田宅</code></div></div></td>
 <td><div class="pattern"><div>破軍</div><div><code>午</code> <code>官祿</code></div></div></td>
 <td><div class="pattern"><div>天機</div><div><code>未</code> <code>僕人</code></div></div></td>
 <td><div class="pattern"><div>紫微 天府</div><div><code>申</code> <code>遷移</code></div></div></td>
 </tr>
 <tr>
 <td><div class="pattern"><div>武曲</div><div><code>辰</code> <code>福德</code></div></div></td>
 <td class="center-palace" rowspan="2" colspan="2">七殺朝鬥</td>
 <td><div class="pattern"><div>太陰</div><div><code>酉</code> <code>疾厄</code></div></div></td>
 </tr>
 <tr>
 <td><div class="pattern"><div>天同</div><div><code>卯</code> <code>父母</code></div></div></td>
 <td><div class="pattern"><div>貪狼</div><div><code>戌</code> <code>財帛</code></div></div></td>
 </tr>
 <tr>
 <td><div class="pattern"><div>七殺</div><div><code>寅</code> <code>命宮</code></div></div></td>
 <td><div class="pattern"><div>天梁</div><div><code>丑</code> <code>兄弟</code></div></div></td>
 <td><div class="pattern"><div>廉貞 天相</div><div><code>子</code> <code>夫妻</code></div></div></td>
 <td><div class="pattern"><div>巨門</div><div><code>亥</code> <code>子女</code></div></div></td>
 </tr>
</table>

<table class="astrolabe">
 <tr>
 <td><div class="pattern"><div>天樑</div><div><code>巳</code> <code>兄弟</code></div></div></td>
 <td><div class="pattern"><div>七殺</div><div><code>午</code> <code>命宮</code></div></div></td>
 <td><div class="pattern"><div></div><div><code>未</code> <code>父母</code></div></div></td>
 <td><div class="pattern"><div>廉貞</div><div><code>申</code> <code>福德</code></div></div></td>
 </tr>
 <tr>
 ...
 <tr>
 <td><div class="pattern"><div>天機 巨門</div><div><code>卯</code> <code>子女</code></div></div></td>
 <td><div class="pattern"><div>破軍</div><div><code>戌</code> <code>官祿</code></div></div></td>
 </tr>
 <tr>
 <td><div class="pattern"><div>貪狼</div><div><code>寅</code> <code>財帛</code></div></div></td>
 <td><div class="pattern"><div>太陽 太陰</div><div><code>丑</code> <code>疾厄</code></div></div></td>
 <td><div class="pattern"><div>武曲 天府</div><div><code>子</code> <code>遷移</code></div></div></td>
 <td><div class="pattern"><div>天同</div><div><code>亥</code> <code>僕人</code></div></div></td>
 </tr>
</table>
...
筆者翻遍《紫微鬥數全書》，也沒有找到這個格局的名字，想來應該是之後的前輩取的名字。破軍是一顆非常有創意的星曜，而且別人不敢說的他敢說，別人不敢做的他敢做。所以成敗的上限和下限都比較大。

其實，星曜之間的個性差異並沒有非常大，特別是現代社會，再特立獨行的人都會被磨平棱角，學會隱藏自己的鋒芒。所以任何的特質，都是相對的。其實從生活中的一些小事，更能看出不同星曜之間的差異。比如說，天同星和破軍星在打麻將的時候，風格差異是非常大的。

對破軍星來說，但凡給他看到一絲絲做大牌的苗頭，他們是絕對不會輕易放過的。而反觀天同星，能贏他們就很開心了。當然，這只是這兩顆星單方面的對比，實際上在不同的星曜組合和四化影響下就算同一顆星也會有不同的表現的。

英星入廟格在商界比較容易有成就，政屆的話要看實際的社會體制。不同的體制、不同的崗位對英星入廟的成就影響很大。小時候成長環境沒有經歷些逆境，對英星入廟來說不一定是好事。

<table class="astrolabe">
 <tr>
 <td><div class="pattern"><div>太陽</div><div><code>巳</code> <code>兄弟</code></div></div></td>
 <td><div class="pattern"><div>破軍</div><div><code>午</code> <code>命宮</code></div></div></td>
 <td><div class="pattern"><div>天機</div><div><code>未</code> <code>父母</code></div></div></td>
 ...
 <td class="center-palace" rowspan="2" colspan="2">英星入廟</td>
 <td><div class="pattern"><div>太陰</div><div><code>酉</code> <code>田宅</code></div></div></td>
 </tr>
 <tr>
 <td><div class="pattern"><div>天同</div><div><code>卯</code> <code>子女</code></div></div></td>
 <td><div class="pattern"><div>貪狼</div><div><code>戌</code> <code>官祿</code></div></div></td>
 </tr>
 <tr>
 <td><div class="pattern"><div>七殺</div><div><code>寅</code> <code>財帛</code></div></div></td>
 <td><div class="pattern"><div>天梁</div><div><code>丑</code> <code>疾厄</code></div></div></td>
 <td><div class="pattern"><div>廉貞 天相</div><div><code>子</code> <code>遷移</code></div></div></td>
 <td><div class="pattern"><div>巨門</div><div><code>亥</code> <code>僕人</code></div></div></td>
 </tr>
</table>
...
<table class="astrolabe">
 <tr>
 <td><div class="pattern"><div>太陰</div><div><code>巳</code> <code>田宅</code></div></div></td>
 <td><div class="pattern"><div>貪狼</div><div><code>午</code> <code>官祿</code></div></div></td>
 <td><div class="pattern"><div>天同 巨門</div><div><code>未</code> <code>僕人</code></div></div></td>
 <td><div class="pattern"><div>武曲 天相</div><div><code>申</code> <code>遷移</code></div></div></td>
 </tr>
 <tr>
 <td><div class="pattern"><div>廉貞 天府</div><div><code>辰</code> <code>福德</code></div></div></td>
 <td class="center-palace" rowspan="2" colspan="2">眾水朝東</td>
 <td><div class="pattern"><div>太陽 天梁</div><div><code>酉</code> <code>疾厄</code></div></div></td>
 </tr>
 <tr>
 <td><div class="pattern"><div></div><div><code>卯</code> <code>父母</code></div></div></td>
 <td><div class="pattern"><div>七殺</div><div><code>戌</code> <code>財帛</code></div></div></td>
 </tr>
 <tr>
 <td><div class="pattern"><div>破軍 文曲</div><div><code>寅</code> <code>命宮</code></div></div></td>
 <td><div class="pattern"><div></div><div><code>丑</code> <code>兄弟</code></div></div></td>
 <td><div class="pattern"><div>紫色微</div><div><code>子</code> <code>夫妻</code></div></div></td>
 <td><div class="pattern"><div>天機</div><div><code>亥</code> <code>子女</code></div></div></td>
 </tr>
</table>
...
<table class="astrolabe">
 ...
 <td><div class="pattern"><div>天同 太陰</div><div><code>午</code> <code>田宅</code></div></div></td>
 <td><div class="pattern"><div>武曲 貪狼</div><div><code>未</code> <code>官祿</code></div></div></td>
 <td><div class="pattern"><div>太陽 巨門</div><div><code>申</code> <code>僕人</code></div></div></td>
 </tr>
 <tr>
 <td><div class="pattern"><div></div><div><code>辰</code> <code>父母</code></div></div></td>
 <td class="center-palace" rowspan="2" colspan="2">眾水朝東</td>
 <td><div class="pattern"><div>天相</div><div><code>酉</code> <code>遷移</code></div></div></td>
 </tr>
 <tr>
 <td><div class="pattern"><div>廉貞 破軍 文曲</div><div><code>卯</code> <code>命宮</code></div></div></td>
 <td><div class="pattern"><div>天機 天梁</div><div><code>戌</code> <code>疾厄</code></div></div></td>
 </tr>
 <tr>
 ...
 <td><div class="pattern"><div>紫微 七殺</div><div><code>亥</code> <code>財帛</code></div></div></td>
 </tr>
</table>
...
它由於組合眾多，無法一一列舉每一種情況，並且依附的星曜也不同，所以三奇加會需要結合具體情況來分析。在眾多組合中，以命宮化科、財帛宮化祿、官祿宮化權這一組最受人們喜愛，讀者應該能看透個中緣由。
...
若嚴格依照同宮才算的話，祿馬交馳只能發生在 [四馬地](./palace.md#四生地)。通常祿馬交馳的宮位都會容易有「停不下來」、忙碌的感覺。是否忙有所獲，當然是要結合其他的星曜組合情況而定啦。不過既然抽到了這個組合，機會一定會比一般人多的，好好抓住時機沖一把，事半功倍，豈不爽哉？
...
那我們反推一下，是否說明祿合鴛鴦的人對理財、經營等比較敏感，直覺比較強呢？就算本命盤沒有抽到這個格局，在運限當中也是會碰到的。所以大家如果遇到鴛鴦祿一定要把握機會。不過也要同時參考其他的星曜組合，把他當作一個參考而不是一個決策的參數。
...
風雲際會是一個行運格局，並不是出生盤上一眼就能找到的固定組合。它的條件是本命身宮或命宮較弱，而大限和小限（也有流派取大限與流年）同時走到 `祿存星`、`化祿星` 或 `天馬星` 所在的有利位置。重點不在於本命夠不夠「貴」，而在於原本受限的盤面，恰好遇到了資源與行動機會一同到位的階段。

《紫微鬥數全書》將它列入雜局，原文是：

> 風雲際會，身命雖弱二限逢祿馬是也。

這個名字很形象：風和雲相遇，才有變化。放到今天，可以理解為基礎條件普通的人，在某一段運程裡得到了流動性、資金、平台或外部機會。能不能借勢而起，仍然要看本命有沒有承接機會的能力；若只是一味求快、沒有準備，祿馬帶來的也可能只是更忙碌的奔波。

## 羊陀夾命

當 `擎羊星` 和 `陀羅星` 分別落在命宮前後兩個相鄰宮位時，就構成羊陀夾命。由於擎羊、陀羅永遠夾著祿存星，所以被夾的命宮同時必有 `祿存星` 坐守；它並不是「命宮同時有羊陀」，也不能把三方會到羊陀誤當成夾命。

這個組合常被直接當成兇格，其實並不嚴謹。祿存提供的是守成、資源與現實感，羊陀則讓這種堅持變成壓力、阻力和不肯退讓。若命宮主星得地、沒有化忌及空劫沖破，往往表現為抗壓、能守、對風險敏感；若再被化忌、空劫或重煞牽動，才容易把壓力變成反覆受阻、關係緊繃或因固執而吃虧。

所以遇到這個格局不必先給自己下結論。與其把它理解為「注定不好」，不如把它當作一個提醒：重要決定要留餘地，資源越緊張的時候越不能賭氣硬扛。

## 馬頭帶箭

馬頭帶箭也常寫作 `馬頭帶劍`。嚴格的成格條件是命宮在 `午宮`，`擎羊星` 與午宮的 `天同太陰` 同宮；命宮空宮而借對宮天同太陰、午宮有擎羊，也可按借星論。丙年或戊年生人時，天同或貪狼得祿的配合較佳；午宮貪狼化祿與擎羊同宮，通常視為旁格。這裡的「馬」是午宮，並不是只要見到天馬星就能成立。
...
> 天同貪羊陀居午位，丙戊鎮御邊疆，為馬頭帶箭富且貴。

天同本身偏重安逸，擎羊卻帶來行動力與競爭壓力；兩者在午宮同度，就像溫和的人被推到了必須出力的位置。古書說「鎮御邊疆」，放到今天更像是適合在高壓、需要執行力和承擔責任的崗位上歷練。它並不是鼓勵衝動，而是說把壓力轉成紀律、技術和行動，反而可能走出成績。

## 左右同宮

左輔星和右弼星同守命宮或身宮，稱為左右同宮；若只是在三方四正會照，只能說有左右拱照，不能按「同宮」論。左輔主助力與規劃，右弼主協調與配合，二星同宮時，互相補足的意味最明顯。

《紫微鬥數全書》說：

> 左右同宮披羅衣紫。

這句古語不必直解為穿紫袍做大官。它更接近於一個人既能組織事情，又能把關係處理得讓人舒服，因此容易在團隊中獲得信任和支持。若主星本身沒有方向，或者煞忌很重，這種「會協調」也可能變成顧慮太多、很難明確表態；仍需結合命宮主星來判斷。

## 左右夾命

左輔星、右弼星分居命宮前後兩個相鄰宮位，即為左右夾命。它和左右同宮不同：同宮偏向個人直接具備輔助與協調能力，夾命則更像成長環境、親友、同事或制度資源在兩側提供支持。左右二星都必須緊鄰命宮，不能以三方會照替代。

《紫微鬥數全書》對此有明確說法：

> 左右夾命為貴格，如安命在丑宮，左輔在子宮，右弼在寅宮。四七十一月生者是也，若不貴則大富。

古人將其說成富貴，是因為左右代表的正是「有人幫、有人接」。現代來看，這種幫助未必是顯赫背景，更可能是關鍵時刻有人願意介紹機會、補足短板，或者所在組織有較好的協作氛圍。個人若不願意溝通、不肯承擔，也很難把這種外部支持變成成果。

## 輔弼拱主

輔弼拱主的核心是 `紫微星` 守命，而左輔、右弼在命宮三方四正拱照；二星分夾紫微命宮也可成立。紫微不在命宮，或只有左輔、右弼其中一顆，都不宜稱為完整的輔弼拱主。

《紫微鬥數全書》說：

> 輔弼拱主，紫微守命二星來拱是也，夾之亦然。

紫微代表統籌和主見，輔弼代表執行與協作，所以這個格局常被解釋為領導力。比起「天生當領導」，更準確的說法是：有意願做主，也比較容易形成支持自己的人。若紫微被煞忌沖破，或本人只想掌控、不願分工，輔弼的好處就會被削弱。

## 魁鉞夾命

天魁星與天鉞星分居命宮前後兩個相鄰宮位，就是魁鉞夾命；它們同宮或在三方會照，雖也有貴人意味，但不屬於這個夾命格。魁鉞的安法由年干決定，所以能否成格必須依出生年和命宮位置逐宮核對。

《紫微鬥數全書》記載：

> 魁鉞夾命為奇格，如命安在辰宮，魁在卯，鉞在巳宮是也。

魁鉞常被稱作貴人星，但貴人不是自動替人完成事情。這個格局更像容易接觸到要求較高、能提供提點的人與環境；個人有專業、講信用，才容易得到提攜。若本命煞忌太重，也可能表現為標準高、壓力大，或總覺得自己需要被認可。

## 坐貴向貴

坐貴向貴指的是天魁、天鉞一星坐命宮，另一星在遷移宮對拱命宮；魁坐命鉞向命，或鉞坐命魁向命，均可成立。二星都在命宮叫魁鉞同宮，二星在三方而不在命遷軸上，則不宜混稱坐貴向貴。

《紫微鬥數全書》的定義很簡潔：

> 坐貴向貴，謂魁鉞在命迭相坐拱是也。

此格局強調的是內外都有助力：自己容易遇到賞識，走出去也比較容易碰到能打開局面的人。把它放在現代，更適合解作善於借力、容易進入優質圈層；真正能留下來，依然取決於能力、作品和長期可靠性。

## 劫空夾命

當地劫星和地空星分別落在命宮前後兩個相鄰宮位時，形成劫空夾命。二星都在命宮、對宮或三方會照，影響當然仍要看，但都不等同於「夾命」。這個格局尤其要分清本命與行運：本命有夾是長期課題，限運臨夾則多是階段性的波動。

《紫微鬥數全書》說：

> 劫空夾命為敗局，假如命安在亥宮，劫在子宮，空在戌宮是也。歲限行到亦凶，夾忌亦凶孤貧刑傷。

地空與地劫容易帶來落差感、計畫落空或資源流失。古人斷得很重，現代不必照單全收，但這確實提示命主在資金、承諾、合作與風險管理上要更重視驗證和留後手。若命宮主星穩健、吉星多，也可能把這種敏感轉化為危機意識與止損能力。

## 祿逢兩殺

祿逢兩殺不是見到祿存和兩顆煞星就成立。按《紫微鬥數全書》的定義，必須是 `祿存星` 坐守命宮而落空亡，並再遇地空、地劫等空劫煞星沖會，重點在「祿被空破」。若祿存只是和一般煞星同度，卻沒有空亡、空劫破壞，不應輕率套用此格。

> 祿逢兩殺，祿坐空亡又逢空劫殺星是也。

祿存代表可積累的資源，空劫則象徵落空與耗散，因此這個格局最需要留意的不是「有沒有錢」，而是收入、儲蓄或機會能否真正留存。凡是高槓桿、口頭承諾、不透明合作，都應多做一層核驗；把帳目、合約和退出機制做好，就是很實際的趨吉避凶。

## 文貴文華

文貴文華指 `文昌星` 與 `文曲星` 同宮；通常以命宮、身宮或命宮三方四正內的同宮組合為取用範圍。文昌偏向條理、學習與表達，文曲偏向感受、審美與呈現，二者結合才有「文貴」「文華」之名。

《紫微鬥數全書》沒有以「文貴文華」四字單列成格，但對昌曲同度多有論述，例如：

> 昌曲祿存猶為奇特。

它並不保證學歷、名聲或職業成就，而是較容易在閱讀、寫作、研究、設計、內容表達、語言溝通等領域顯出優勢。昌曲遇煞忌時，也可能把敏感與才氣用成虛榮、猶豫或口舌；持續練基本功，才是把才華變成成果的關鍵。

## 文星朝命

文星朝命也常稱文星拱命，指文昌、文曲在命宮三方四正會照命宮；兩星同宮於命宮當然更強。通常還要求命宮及三方沒有重煞、化忌沖破，否則只能說有文星，不能簡單按吉格下結論。

《紫微鬥數全書》中有相近的說法：

> 文科拱照，賈誼年少登科。

古代將「登科」看得很重，今天則可以轉譯為學習能力、文字表達、專業認證和公開呈現較容易得到發揮。尤其適合需要長期累積知識、把複雜問題說明白的工作；但這不等於不用努力，文星最怕眼高手低，或只停留在會說不會做。

## 昌曲夾命

文昌星與文曲星分別落在命宮前後兩個相鄰宮位，即為昌曲夾命。命宮本身宜有吉星或主星得地，並避免重煞沖破；如果被夾的恰是空劫、化忌等，文星的清秀和表達力也容易變得不穩定。二星只在三方會照時，是文星朝命，不是昌曲夾命。

《紫微鬥數全書》說：

> 昌曲夾命最為奇，假若命在丑宮，文昌在寅，文曲在子是也。不貴即富，吉多方論此為貴。

昌曲從兩側夾命，常見的不是「自動考第一」，而是環境重視教育、溝通與審美，自己也較容易形成文字、表達或專業整理的能力。把它用在寫作、研究、產品、設計、教學等需要長期打磨的地方，比拿它當考試保送券更實際。

## 文星暗拱

文星暗拱通常指文昌、文曲在命宮前後夾照，亦即昌曲夾命；也有流派將文昌文曲在遷移宮正照命宮、或在命宮三方會照時稱作暗拱。名稱和口徑並不完全一致，實務上應先寫清星曜是「夾」「對拱」還是「三合會照」，不要只報格名。

《紫微鬥數全書》有如下斷語：

> 文星暗拱，賈誼允矣登科。

這類組合的重點仍是知識、表達與被看見的機會。對於現代人來說，比較貼近的表現是更容易透過作品、證書、演講、論文或專業內容取得認可；如果沒有持續輸出，單有文星也不會自動變成履歷。

## 權祿生逢

權祿生逢指 `化權星` 與 `化祿星` 同守命宮，並且所依附的主星處於廟、旺等較有力的位置；若主星落陷，或權祿不在命宮而只是三方會照，都不宜按這個嚴格名稱論。它關注的是資源（祿）和推動、掌控力（權）在同一處發揮。

《紫微鬥數全書》的原文是：

> 權祿生逢，二星守命廟旺是也，陷不是。

現代語境裡，它容易表現為既想把事情做大，也願意為結果負責。優點是目標感和調度資源的能力較強；短板則是容易把效率、利益和控制欲綁得太緊。權祿若再遇煞忌，仍要注意權責不清、急於求成或為了短期收益透支信用。

## 科明暗祿

科明暗祿也稱 `明珠暗祿`。嚴格條件是 `化科星` 坐命宮，而命宮的暗合宮（六合位）有 `祿存星`；部分流派也接受暗合宮為化祿，但採用哪一種必須先說明。化科只是三方會照、祿也不在暗合宮時，不算此格。

《紫微鬥數全書》定義為：

> 科明暗祿，科明祿暗格，即化科守命宮，命宮之暗合宮有祿存是也。

化科偏向名譽、專業與可見的認可，暗祿則像不那麼張揚的資源支持。因此它更適合解讀為：透過專業信譽得到機會，或在看不見的渠道獲得助力。不要把「暗祿」理解成必有秘密橫財，任何收益都仍要經得起合規與事實檢驗。

## 科權祿夾

化祿、化權、化科三吉化中，至少兩顆分居命宮前後兩個相鄰宮位來夾命，稱為科權祿夾；三顆中有兩顆夾命即可，三顆都集中在鄰宮而沒有形成左右夾照則不算。命宮主星廟旺、少煞忌時，格局的正面作用較容易發揮。

《紫微鬥數全書》有明確例子：

> 科權祿夾為貴格，如命安在子宮，祿在亥宮，權在丑宮，為夾貴，餘作此論。

把它說成「貴」，是因為兩側同時給命宮帶來資源、執行力和名望的加持。現代可理解為較容易得到制度、團隊或家人兩邊的支持；不過夾宮主要反映環境與關係條件，命宮本身是否願意承擔、能否行動，仍然是決定性的部分。

## 甲第登庸

甲第登庸指 `化科星` 坐守命宮，而 `化權星` 在遷移宮或命宮三方四正朝拱命宮；若再會化祿或祿存，傳統上認為更完整。核心並不是只見到一顆化科或化權，而是「科在命、權來朝」，使能力、名聲與執行機會可以互相配合。

《紫微鬥數全書》說：

> 科命權朝登庸甲第，或權或祿全更佳，為權祿逢迎格。

「甲第」是科舉名次，「登庸」是被任用。放在今天，可以理解為專業能力被認可後，有機會進入更能承擔責任的位置。這是一個鼓勵把學識轉化為實踐的組合：只有證書而沒有執行力，或者只有野心而沒有專業累積，都很難把古書裡的美好斷語落實出來。
