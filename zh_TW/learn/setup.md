...
例如壬辰年卯時生人，據`申子辰人寅戌揚`口訣，故火星在寅宮起子時，鈴星在戌宮起子時，順數至卯時，即火星在巳，鈴星在丑。
...
:::warning 註意
流曜一般只取 `大限` 和 `流年` 的星曜，流月、流日、流時的一般不看，所以本站程序也只提供到大限和流年的流曜。一般來說，`大限`的流曜會在星曜前面加上 `運` 字，`流年` 的流曜會在星曜前面加上 `流` 字，以便區分。比如大限的 `天馬` 叫 `運馬`，`流年` 的 `天馬` 叫 `流馬`。
:::
